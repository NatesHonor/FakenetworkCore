...
public class DenyReportCommand extends Command {
    Core core = Core.getInstance();

    public DenyReportCommand() {
        super("denyreport");
    }
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args.length != 1) {
            sender.sendMessage(new TextComponent(ChatColor.RED + "Usage: /denyreport <report_id>"));
            return;
        }
        ...
        try {
            ...
            if (resultSet.next()) {
                ...
                PreparedStatement insertStatement = core.getConnection().prepareStatement(
                        "INSERT INTO denied_reports (reporter_name, reported_name, reason) VALUES (?, ?, ?)");
                ...
                sender.sendMessage(
                        new TextComponent(ChatColor.GREEN + "Report ID " + reportId + " has been denied and removed."));
            } else {
            ...
            e.printStackTrace();
            sender.sendMessage(new TextComponent(ChatColor.RED + "An error occurred while denying the report."));
        }
    }
}
