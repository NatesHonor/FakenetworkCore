...
public class MuteDisrespect extends Command {

    public MuteDisrespect() {
        super("mdisrespect");
    }
    ...
    public void execute(CommandSender sender, String[] args) {
        if (sender instanceof ProxiedPlayer && sender.hasPermission("fakenetwork.staff")) {
            ...
            int offenseCount = mutes.getOffenseCount(targetPlayerName, "Disrespect");
            String reason = "Disrespect" + (offenseCount > 0 ? " (" + (offenseCount + 1) + ")" : "");
            int durationInDays = 2 * (offenseCount + 1); // Duration is 2 days for each offense
            ...
            if (targetPlayer != null) {
                targetPlayer.sendMessage(
                        new TextComponent("You have been muted for disrespect for " + durationInDays + " day(s)."));
                if (offenseCount > 0) {
                    targetPlayer.sendMessage(new TextComponent("Since this is offense # " + (offenseCount + 1)
                            + ", your mute has been extended by " + offenseCount * 2 + " day(s)."));
                }
            }
            ...
            sender.sendMessage(
                    new TextComponent("Player '" + targetPlayerName + "' has been muted for disrespect for "
